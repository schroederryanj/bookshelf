...
import { ReadingStreakBadge } from "./reading-progress/ReadingStreakBadge";

type EnhancedStatsProps = {
  books: Book[];
  currentStreak?: number;
  longestStreak?: number;
};

export function EnhancedStats({ books, currentStreak = 0, longestStreak = 0 }: EnhancedStatsProps) {
  const totalBooks = books.length;
  const totalReadBooks = books.filter((book) => book.read === "Read").length;
  const currentlyReadingBooks = books.filter((book) => book.read === "Reading").length;
  ...
  return (
    <div className="w-full flex justify-center mb-6">
      <div
        className="max-w-5xl w-full mx-4 overflow-hidden"
        style={{
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="lg:col-span-3 space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                {/* Currently Reading */}
                <div className="text-center p-3 rounded-lg bg-white/40">
                  <div className="text-3xl font-bold text-[#8b5a2b]">{currentlyReadingBooks}</div>
                  <div className="text-xs text-[#6b5a4a] uppercase tracking-wider mt-1">Reading</div>
                </div>
              ...
              {averageRating && (
                <div className="flex items-center justify-center gap-2 p-3 rounded-lg bg-white/40">
            ...
            {/* Reading Streak Badge - Takes up 1 column */}
